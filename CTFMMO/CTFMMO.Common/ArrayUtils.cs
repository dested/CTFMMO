...
namespace CTFMMO.Common
{
    public static class EnumerableExtensions
    {
        ...
        public static T[] OrderBy<T, T2>(this T[] items, Func<T, T2> clause) where T2 : IComparable<T2>
        {
            var j = items.Clone();
            j.Sort((a, b) => clause(a).CompareTo(clause(b)));
            return j;
        }
        public static T[] OrderBy<T, T2>(this List<T> items, Func<T, T2> clause) where T2 : IComparable<T2>
        {
            var j = items.Clone();
            j.Sort((a, b) => clause(a).CompareTo(clause(b)));
            return j.ToArray();
        ...
        public static int Count<T>(this IEnumerable<T> items, Func<T, bool> clause)
        {
            var items2 = new List<T>();
